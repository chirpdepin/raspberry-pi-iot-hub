...
async function run() {
    ...
    for (const label of EXPECTED_NAV) {
        check(`nav shows "${label}"`, navText.some((t) => t.includes(label)));
    }

    const rootHtml = await window.webContents.executeJavaScript(
    ...
    const bg = await window.webContents.executeJavaScript(
        `getComputedStyle(document.body).backgroundColor`,
    );
    check('theme applied to body', bg !== '' && bg !== 'rgba(0, 0, 0, 0)', bg);

    const fontFamily = await window.webContents.executeJavaScript(
