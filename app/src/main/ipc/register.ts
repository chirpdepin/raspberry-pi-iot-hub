...
export const registerIpcHandlers = (deps: IpcDependencies): void => {
  ...
  // `null` rather than undefined over the wire: structured clone preserves it,
  // and an absent argument would be indistinguishable from a dropped one.
  ipcMain.handle(IPC.cameraAdd, async (_event, camera: DiscoveredCameraPayload | null) =>
    handleCameraAdd(deps.cameraAdd, camera ?? undefined)
  );
